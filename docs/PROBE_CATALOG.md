...
The last row is composed cross-tool: NQ's internal observation of process + listener presence vs. a local HTTP probe of the same listener. That composition belongs to NQ's SQL workbench, not to blackbox itself — see `notquery/docs/coverage/sql-composed-checks.md`.
