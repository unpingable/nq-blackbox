...
**PROMOTED 2026-07-03 (sushi-k prove-the-wheel):** `nq_aggregator_http_localhost` (`http_2xx`) is promoted from the local vantage — all five criteria met against a **live** `blackbox_exporter` v0.28.0 (`127.0.0.1:9115`, user-systemd `nq-blackbox.service`) scraped by sushi-k's NQ witness. `blackbox.yml` module + `targets/local.yml` entry landed; `scripts/smoke_probe.sh` SMOKE PASS (`probe_success=1` ingested as `scrape_target_name='nq_aggregator_http_localhost'`, `scrape_target_collision=0`, content intact). Live-verified the collision caveat's boundary: the http prober's multi-phase `probe_http_duration_seconds` rows carry a distinguishing `phase` label and stay collision-free, while a second BARE-`probe_success` target (e.g. `dns_neutralzone_localhost`) would collapse — so it stays a commented candidate in `targets/local.yml` pending the identity migration.

**PROMOTED 2026-07-03 (VM deploy):** `external_http_nq_neutralzone` (`http_2xx`, Bucket 3) promoted from the VM — `blackbox_exporter` v0.28.0 (system-systemd `nq-blackbox.service`, `127.0.0.1:9115`) scraped by the VM's NQ witness. `targets/linode.yml` entry landed; `smoke_probe.sh` SMOKE PASS (`probe_success=1` ingested as `scrape_target_name='external_http_nq_neutralzone'`, `collision=0`). The probe exercises the full public-path stack (DNS → Caddy → TLS 1.3 → public bind; valid Let's Encrypt cert `CN=nq.neutral.zone` observed via `probe_ssl_earliest_cert_expiry`) — but the vantage is the VM itself, not a foreign network (see `targets/linode.yml` vantage-honesty note; `tls_expiry_nq_neutralzone` cert-expiry testimony is available in the same samples but not separately promoted). One clean bare-`probe_success` target per island (series tables are per-host), so `labelwatch_health_localhost` stays a commented candidate pending the identity migration.

**Fleet blackbox state:** live exporters on sushi-k (`nq_aggregator_http_localhost`) + VM (`external_http_nq_neutralzone`). NAS has no exporter (no forcing case). The multi-probe-per-host unlock is the deferred scrape-target identity migration (`nq/docs/working/decisions/NQ_SCRAPE_TARGET_IDENTITY_SCOPE.md`).
